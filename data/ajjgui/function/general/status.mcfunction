...
execute if score #bool ajjgui.temp matches 1 at @e[type=minecraft:marker,tag=ajjgui.box] run function ajjgui:general/install_success
execute if score #bool ajjgui.temp matches 1 run tellraw @a [{text:"\nUpdated "},{text:"ajjgui",color:"gray"},{text:" to version "},{storage:"ajjgui:data",nbt:"version",interpret:true,color:"gray"}]
